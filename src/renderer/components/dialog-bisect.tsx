...
import { filterItem, getItemIcon, renderItem } from './commands-version-chooser';
...
export class BisectDialog extends React.Component<BisectDialogProps, BisectDialogState> {
  constructor(props: BisectDialogProps) {
    ...
    const allVersions = this.getItems();
  ...
  public render() {
    ...
    return (
      ...
      >
        ...
        <Label>
          Latest Version
          {console.log(this.getItems().slice(startIndex))}
          <ElectronVersionSelect
            filterable={true}
            items={this.getItems().slice(0, startIndex!)}
            itemRenderer={renderItem}
            itemPredicate={filterItem}
